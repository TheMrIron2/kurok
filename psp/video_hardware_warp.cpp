...
//	pcx_rgb = malloc( count * 4);
    pcx_rgb = static_cast<byte*>(malloc(count*4));
...
void Sky_LoadSkyBox (char *name)
{
    ...
    // sky palette
    sprintf (skypalette, "gfx/env/%s%spalette.lmp", name, size);
    ...
    VID_InitPaleteSKY(host_skybasepal);

    for (i=0 ; i<6 ; i++)
    {
        mark = Hunk_LowMark ();
        sprintf (filename, "gfx/env/%s%s%s.lmp", name, size, suf[i]);
        f = static_cast<byte*>(COM_LoadHunkFile (filename));

#ifdef  SLIM_MODEL
        if (f) // Load up 512 x 512 sky textures if on slim psp.
        {
            if (!Q_strncmp(size,"_512_",5))
